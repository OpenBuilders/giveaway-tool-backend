...
func (s *giveawayService) GetChannelAvatar(ctx context.Context, channelID int64) (string, error) {
	if s.debug {
		log.Printf("[DEBUG] Getting channel avatar for channel %d", channelID)
	}

	key := fmt.Sprintf("channel:%d:avatar", channelID)
...
func (s *giveawayService) GetUserChannels(ctx context.Context, userID int64) ([]int64, error) {
	if s.debug {
...
func (s *giveawayService) GetChannelTitle(ctx context.Context, channelID int64) (string, error) {
	if s.debug {
		log.Printf("[DEBUG] Getting title for channel %d", channelID)
	}

	key := fmt.Sprintf("channel:%d:title", channelID)
...
func (s *giveawayService) GetPublicChannelInfo(ctx context.Context, username string) (*telegram.PublicChannelInfo, error) {
